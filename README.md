...
The INR model builds on the work of Zha, R., et al. [1] ([NAF](https://github.com/Ruyi-Zha)) with modifications described in our paper. The fast diffusion model builds on the work of Nichol, A. Q. and Dhariwal, P. [2] ([DDPM](https://github.com/openai/improved-diffusion)). We accelerated the sampling process by incorporating modifications proposed in [3] ([fastWDM](https://github.com/AliciaDurrer/fastWDM3D))
...
1. In [`naf/dataGenerator`](naf/dataGenerator/), prepare the training data for the INR by running
...
Run an iterative reconstruction with the INR rendered projections with:
   ```bash
   python MMDental_reconstruction.py
   ```
