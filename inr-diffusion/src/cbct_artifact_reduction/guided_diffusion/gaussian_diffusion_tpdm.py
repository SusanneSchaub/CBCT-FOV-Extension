...
class GaussianDiffusion:
    ...
    def p_sample_loop_inpainting(
        ...
        noise = th.randn_like(cond1).to(device)
        cond1 = cond1.to(device)
        x_noisy = th.cat((noise, cond1), dim=1).float()
    ...
    def p_sample_loop_progressive_inpainting(
        ...
        generated_vol = th.zeros((1,1,256,256,256)).to(device)
            ...
            if i % 2 == 0:
                ...
                for z in range(256):
                    ...
                    if input.shape[1] != 2:
                        ...
                        input = th.cat((input, cond1[:,:,z,:,:]), dim=1).float()
                    with th.no_grad():
                        ...
                        output = out["sample"]  # This is the reduced noise image and has 1 channel

                    generated_vol[:,:,z,:,:] = output
                    if z == 100:
                        #viz.image(visualize(output))
                        ok = 1
                    img = generated_vol.clone()
                    yield {"sample": generated_vol.clone()}
