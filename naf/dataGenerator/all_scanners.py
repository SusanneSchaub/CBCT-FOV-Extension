...
def main():
    ...
    if scanner == 'planmeca':
        nbrs = [60,140,220,300,380,442,498,554,610,666,683,687,691,695,699,703]

    if scanner == 'axeos':
        nbrs = [20,100,180,260,340,414,470,526,582,638,681,685,689,693,697,701]

    if scanner == 'accuitomo':
        nbrs = [40,120,200,280,360,428,484,540,596,652,682,686,690,694,698,702]

    if scanner == 'x800':
        nbrs = [80,160,240,320,400,456,512,568,624,680,684,688,692,696,700,704]



    for nbr in nbrs:

        configPath = "/home/naf_meta_learned/dataGenerator/raw/jaw/config_" + scanner + ".yml"

        outputPath = osp.join(outputFolder, f"{str(nbr)}_projections.pickle")
        generator(matPath, configPath, outputPath, nbr, scanner,True)
...
def loadImage(dirname, nVoxels, convert, rescale_slope, rescale_intercept, nbr, scanner, normalize=True):
    """
    Load CT image.
    """
    ...
    pth = "/raid/cian/susanne.schaub/meta_learn_volumes/raw_volumes/" + scanner + "/" + str(nbr) + ".nii.gz"
    pth_nbr = pth.split('/')[7]
    pth_nbr = int(pth_nbr.split('.')[0])
    ...
    if ((scanner == 'x800') ):
        ...
        if (pth_nbr == 80):
            test_data = test_data[:, :, 30:test_data.shape[2] -20]
        if (pth_nbr == 160):
            test_data = test_data[:, :, 30:test_data.shape[2] - 20]
        if (pth_nbr == 240):
            test_data = test_data[:, :, 20:test_data.shape[2] - 20]
        if (pth_nbr == 320):
            test_data = test_data[:, :, 10:test_data.shape[2] - 40]
        if (pth_nbr == 400):
            test_data = test_data[:, :, 20:test_data.shape[2] - 30]
        if (pth_nbr == 456):
            test_data = test_data[:, :, 30:test_data.shape[2] - 20]
        if (pth_nbr == 512):
            test_data = test_data[:, :, 45:test_data.shape[2] - 0]
        if (pth_nbr == 568):
            test_data = test_data[:, :, 20:test_data.shape[2] - 20]
        if (pth_nbr == 624):
            test_data = test_data[:, :, 30:test_data.shape[2] - 10]
        if (pth_nbr == 680):
            test_data = test_data[:, :, 45:test_data.shape[2] - 0]

        if (pth_nbr == 684):
            test_data = test_data[:, :, 80:test_data.shape[2]]
        if (pth_nbr == 688):
            test_data = test_data[:, :, 60:test_data.shape[2]]
        if (pth_nbr == 692):
            test_data = test_data[:, :, 80:test_data.shape[2]]
        if (pth_nbr == 696):
            test_data = test_data[:, :, 90:test_data.shape[2]]
        if (pth_nbr == 700):
            test_data = test_data[:, :, 90:test_data.shape[2]]
        if (pth_nbr == 704):
            test_data = test_data[:, :, 80:test_data.shape[2]]
    ...
    if scanner == 'planmeca':
        ...
        if (pth_nbr == 60):
            test_data = test_data[:, :, 200:test_data.shape[2] - 240]
        if (pth_nbr == 140):
            test_data = test_data[:, :, 190:test_data.shape[2] - 215]
        if (pth_nbr == 220):
            test_data = test_data[:, :, 180:test_data.shape[2] - 200]
        if (pth_nbr == 300):
            test_data = test_data[:, :, 180:test_data.shape[2] - 215]
        if (pth_nbr == 380):
            test_data = test_data[:, :, 170:test_data.shape[2] - 225]
        if (pth_nbr == 442):
            test_data = test_data[:, :, 100:test_data.shape[2] - 60]
        if (pth_nbr == 498):
            test_data = test_data[:, :, 120:test_data.shape[2] - 35]
        if (pth_nbr == 554):
            test_data = test_data[:, :, 50:test_data.shape[2] - 100]
        if (pth_nbr == 610):
            test_data = test_data[:, :, 130:test_data.shape[2] - 30]
        if (pth_nbr == 666):
            test_data = test_data[:, :, 30:test_data.shape[2] - 160]
        if (pth_nbr == 683):
            test_data = test_data[:, :, 150:test_data.shape[2] - 36]
        if (pth_nbr == 687):
            test_data = test_data[:, :, 150:test_data.shape[2] - 36]
        if (pth_nbr == 691):
            test_data = test_data[:, :, 110:test_data.shape[2] - 70]
        if (pth_nbr == 695):
            test_data = test_data[:, :, 50:test_data.shape[2] - 110]
        if (pth_nbr == 699):
            test_data = test_data[:, :, 150:test_data.shape[2] - 35]
        if (pth_nbr == 703):
            test_data = test_data[:, :, 180:test_data.shape[2] - 35]
    ...
    if ((scanner == 'accuitomo')):
        ...
        if (pth_nbr == 40):
            test_data = test_data[:, :, 20:test_data.shape[2] -40]
        if (pth_nbr == 120):
            test_data = test_data[:, :, 0:test_data.shape[2] - 40]
        if (pth_nbr == 200):
            test_data = test_data[:, :, 0:test_data.shape[2] - 40]
        if (pth_nbr == 280):
            test_data = test_data[:, :, 20:test_data.shape[2] - 30]
        if (pth_nbr == 360):
            test_data = test_data[:, :, 0:test_data.shape[2] - 50]
        if (pth_nbr == 428):
            test_data = test_data[:, :, 0:test_data.shape[2] - 60]
        if (pth_nbr == 484):
            test_data = test_data[:, :, 0:test_data.shape[2] - 60]
        if (pth_nbr == 540):
            test_data = test_data[:, :, 60:test_data.shape[2] - 0]
        if (pth_nbr == 596):
            test_data = test_data[:, :, 20:test_data.shape[2] - 40]
        if (pth_nbr == 652):
            test_data = test_data[:, :, 0:test_data.shape[2] - 40]
        ...
        if (pth_nbr == 682):
            test_data = test_data[:, :, 50:test_data.shape[2] - 10]
        if (pth_nbr == 686):
            test_data = test_data[:, :, 80:test_data.shape[2]]
        if (pth_nbr == 690):
            test_data = test_data[:, :, 100:test_data.shape[2]]
        if (pth_nbr == 694):
            test_data = test_data[:, :, 110:test_data.shape[2]]
        if (pth_nbr == 698):
            test_data = test_data[:, :, 130:test_data.shape[2]]
        if (pth_nbr == 702):
            test_data = test_data[:, :, 90:test_data.shape[2]]
    ...
    if ((scanner == 'axeos')):
        ...
        if (pth_nbr == 20):
            test_data = test_data[:, :, 160:test_data.shape[2] - 140]
        if (pth_nbr == 100):
            test_data = test_data[:, :, 200:test_data.shape[2] - 80]
        if (pth_nbr == 180):
            test_data = test_data[:, :, 250:test_data.shape[2] - 20]
        if (pth_nbr == 260):
            test_data = test_data[:, :, 210:test_data.shape[2] - 30]
        if (pth_nbr == 340):
            test_data = test_data[:, :, 290:test_data.shape[2] - 10]
        if (pth_nbr == 414):
            test_data = test_data[:, :, 280:test_data.shape[2] - 0]
        if (pth_nbr == 470):
            test_data = test_data[:, :, 330:test_data.shape[2] - 0]
        if (pth_nbr == 526):
            test_data = test_data[:, :, 290:test_data.shape[2] - 0]
        if (pth_nbr == 582):
            test_data = test_data[:, :, 300:test_data.shape[2] - 0]
        if (pth_nbr == 638):
            test_data = test_data[:, :, 210:test_data.shape[2] - 10]

        if (pth_nbr == 681):
            test_data = test_data[:, :, 300:test_data.shape[2] - 20]
        if (pth_nbr == 685):
            test_data = test_data[:, :, 280:test_data.shape[2] - 150]
        if (pth_nbr == 689):
            test_data = test_data[:, :, 280:test_data.shape[2] - 130]
        if (pth_nbr == 693):
            test_data = test_data[:, :, 280:test_data.shape[2] - 140]
        if (pth_nbr == 697):
            test_data = test_data[:, :, 300:test_data.shape[2] - 130]
        if (pth_nbr == 701):
            test_data = test_data[:, :, 300:test_data.shape[2] - 150]


    viz.image(visualize(test_data[..., 0]), opts=dict(title=str(pth_nbr)))
    viz.image(visualize(test_data[..., -1]), opts=dict(title=str(pth_nbr)))
...
def generator(matPath, configPath, outputPath, nbr, scanner, show=False):
    """
    ...
    img = loadImage(matPath, data["nVoxel"], data["convert"],data["rescale_slope"], data["rescale_intercept"], nbr, scanner, data["normalize"])
    data["image"] = img.copy()


    angle_z = np.linspace(0, 2 * np.pi, 500)

    geo_SID = np.zeros(0,500)

    geo_SID = np.zeros_like(geo_SID)+data["DSD"]
    geo_SOD = np.zeros_like(geo_SID)+data["DSO"]
    ...
    #imgFDK = algs.fdk(projections_np[:, ::-1, :], geo, angle_z)
    #breakpoint()
