...
class ConeGeometry(object):
    ...
    def __init__(self, data, prefix):
        ...
        # OffsetsData
        self.offOrigin = np.array(data["offOrigin"][::-1]) / 1000 #np.array([0,0,0]) # Offset of image from origin   (m)
...
class TIGREDataset(Dataset):
    ...
    def __getitem__(self, index):
        if self.type == "train":
            ...
            for stp in range(self.inner_stp): #add +1 for meta learning!
                ...
                #for tv calculation of outpainted region
                #select_inds_tv_out = np.random.choice(coords_valid_tv_out.shape[0], size=[self.n_rays], replace=False)
                #select_coords_tv_out = coords_valid[select_inds_tv_out].long()
                ...
                #here stack several rays and projs together (keep same index) (for tv calculation of outpainted region)
                #rays_tv_out = self.rays[index, select_coords_tv_out[:, 0], select_coords_tv_out[:, 1]]
                #projs_tv_out = self.projs[index, select_coords_tv_out[:, 0], select_coords_tv_out[:, 1]]       #dont change!!
                #list_rays_tv_out.append(rays_tv_out)
                #list_proj_tv_out.append(projs_tv_out)
    ...
    def get_rays(self, angles, geo: ConeGeometry, jj, device, mode, save=False):
        ...
        if mode == 'normal':
            ...
            H,W = geo.nDetector #now all projections have original shape #145, 230 #512, 512 #145,230 #250,800 #560,560 #outpaint
        rays = []
