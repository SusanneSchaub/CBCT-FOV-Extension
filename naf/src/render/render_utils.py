...
def cast_rays(idx_epoch, rays, radii, std_scale, n_samples, n_fine, perturb, netchunk, raw_noise_std):
  ...
  """
    ...
    t0 = t_vals[:-1] #t0 = tdist[..., :-1, None]
    t1 = t_vals[1:] #t1 = tdist[..., 1:, None]
    #radii = radii[..., None]

    t_m = (t0 + t1) / 2
    t_d = (t1 - t0) / 2

    j = torch.arange(6, device=t_vals.device)
    pdb.set_trace()
    t = t0 + t_d / (t_d ** 2 + 3 * t_m ** 2) * (t1 ** 2 + 2 * t_m ** 2 + 3 / 7 ** 0.5 * (2 * j / 5 - 1) * (
        (t_d ** 2 - t_m ** 2) ** 2 + 4 * t_m ** 4).sqrt())

    deg = torch.pi / 3 * torch.tensor([0, 2, 4, 3, 5, 1], device=t_vals.device, dtype=torch.float)
    ...
    if rand:
        ...
        mask = torch.arange(t.shape[-2], device=t_vals.device) % 2 == 0
    ...
    rand_vec = torch.randn_like(rays_d)
    ortho1 = F.normalize(torch.cross(rays_d, rand_vec, dim=-1), dim=-1)
    ortho2 = F.normalize(torch.cross(rays_d, ortho1, dim=-1), dim=-1)
    ...
    basis_matrix = torch.stack([ortho1, ortho2, rays_d], dim=-1)
