...
class ActivatedLinear(nn.Module):
    ...
    def __init__(self, in_features, out_features, activation=None, layer_type="hidden", init_siren=False,
                 omega_for_init=None, bias_scale=0.0, init_bias=False, init_mfn=False, weight_scale=1.0):
        super().__init__()
...
class SirenMLP(nn.Module):
    ...
    def __init__(self, input_dim=2, hidden_dim=256, output_dim=3, num_layers=4, omega=30.0):
        ...
        layers = [
            ActivatedLinear(input_dim, hidden_dim, SinusoidalActivation(omega=self.omega_values[0]), layer_type="first",
                            init_siren=True)]
        for i in range(num_layers - 2):
            layers.append(ActivatedLinear(hidden_dim, hidden_dim, SinusoidalActivation(omega=self.omega_values[i + 1]),
                                          layer_type="hidden", init_siren=True))
        # DO NOT CHANGE: we use ActivatedLinear to get the spectral norm info, even without activation
