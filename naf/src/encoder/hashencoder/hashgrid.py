...
class HashEncoder(nn.Module):
    def __init__(self, input_dim=3, num_levels=16, level_dim=2, base_resolution=16, log2_hashmap_size=19, init_array = None):
        super().__init__()
    ...
    def init_grid(self, init_array, voxel_space, bound = 1):
        w, h, l = voxel_space
        for i, emb_pos in enumerate(self.offsets):
            if i == 0:
                emb_size = emb_pos
            else:
                emb_size = emb_pos-self.offsets[i-1]
...
def functional_hash_encoder(
    ...
    """
    Functional forward for HashEncoder.

    Args:
        encoder: HashEncoder instance (structure + buffers)
        inputs: [..., input_dim]
        params: parameter dict containing 'encoder.embeddings'
        size: normalization bound
    ...
    prefix_shape = list(inputs.shape[:-1])
    inputs = inputs.view(-1, encoder.input_dim)

    # critical line: use params, not encoder.embeddings
    embeddings = params["encoder.embeddings"]

    outputs = hash_encode(
        inputs,
        embeddings,
        encoder.offsets,
        encoder.base_resolution,
        inputs.requires_grad,
    )

    outputs = outputs.view(prefix_shape + [encoder.output_dim])
    return outputs
